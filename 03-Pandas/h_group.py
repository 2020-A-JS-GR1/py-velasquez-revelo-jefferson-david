# -*- coding: utf-8 -*-
"""
Created on Sat Aug 15 09:56:24 2020
...
#artista y el dataframe del artista agrupado
for columna,df_agrupado in df_agrupar_artista:
    print(type(columna))
    print(columna) #artista
    print(type(df_agrupado))
    print(df_agrupado) # un dataframe
    
#hace calculos en columnas del df
    
a = seccion_df['units'].value_counts() #38 y 1 nan
#deberian mostrarse los nan

#emtpy sirve para verificar si la columna esta vacia
print(a.empty)
...
def llenar_valores_vacios(series, tipo):
    ...
    if(lista_valores.empty == True):
        ...
        if(tipo == 'promedio'):
            ...
            return series.fillna(promedio)
        if(tipo == 'mas_repetido'):
            repetido = lista_valores.reset_index().head(1)['index'][0]
            return series.fillna(repetido)
